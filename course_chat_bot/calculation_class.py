...
class Calculations:
    ...
    def get_corona_data_by_date(date):  # Responce
        ...
        try:
            if date.day >= 10:
                if date.month >= 10:
                    req = requests.get(f'{mainurl_corona}{str(date.month)}-{str(date.day)}-2020.csv')
                else:
                    req = requests.get(f'{mainurl_corona}0{str(date.month)}-{str(date.day)}-2020.csv')
            else:
                if date.month > 10:
                    req = requests.get(f'{mainurl_corona}{str(date.month)}-0{str(date.day)}-2020.csv')
                else:
                    req = requests.get(f'{mainurl_corona}0{str(date.month)}-0{str(date.day)}-2020.csv')
            return req
    ...
    def get_position_weather():
        req = requests.get(
            'https://yandex.ru/pogoda/47?utm_source=serp&utm_campaign' 
            '=wizard&utm_medium=desktop&utm_content=wizard_desktop_main&utm_term=title')
        ...
        position = html_weather.rfind(
            ...
            '<span class="temp__pre-a11y a11y-hidden">Текущая температура</span><span class="temp__value">') + len(
            '<div class="temp fact__temp fact__temp_size_s" role="text">'
            '<span class="temp__pre-a11y a11y-hidden">Текущая температура</span><span class="temp__value">')
        if position < 0:
            return None
        else:
